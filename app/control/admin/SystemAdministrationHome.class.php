 ...
 */
class SystemAdministrationHome extends TPage
{
    /**
     * Class constructor
     * Creates the page
    ...
    function __construct()
    {
        ...
        try
        {
            ...
            $stats1 = SystemUserGroup::groupBy('system_contract_id')->countBy('system_user_id', 'count');
            if ($stats1)
            {
                foreach ($stats1 as $row)
                {
                    $data1[] = [ SystemGroup::find($row->system_contract_id)->status, (int) $row->count];
                }
            }
